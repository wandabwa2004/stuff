...
runRFmodel <- function(data, excelFile = "ModelOutput.xlsx", sample_training = 0.7
  ...
  list.of.packages <- c("ggplot2", "xlsx","Hmisc","randomForest","ROCR")
  ...
  cat("Finished\n\nTraining data split over the target as:")
  ...
  set.seed(42); data.test$predProb <- predict(data.rf, data.test, type="prob")
  set.seed(42); data.test.rf.pred <- prediction(data.test$predProb[,2], data.test$Target)  
  
  set.seed(42); data.train$predProb <- predict(data.rf, data.train, type="prob")
  ...
  results <- list("confusion"=data.test.confusion, "evaluation"=evaluation, "importance"=importance
                  ,"model"=data.rf, "precision_recall"=precision_recall, "tpr_fpr"=tpr_fpr)
